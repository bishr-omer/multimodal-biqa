...
INPUT_SIZE = 224  # ViT-H/14 native resolution
...
def multi_crop_features(pil_img):
    ...
    tensors = torch.stack([crop_preprocess(c) for c in crops]).to(device)  # (6, 3, 224, 224)
    with torch.no_grad():
        feats    = model.encode_image(tensors)           # (6, 1024)
...
def extract_features(image_folder, output_path):
    ...
    print(f"Model : ViT-H-14 (laion2b_s32b_b79k)  dim=1024")
    print(f"Device: {device}")

    names, features = [], []
    for img_path in tqdm(image_paths, desc="Extracting CLIP-H/14 features"):
        try:
    ...
    print(f"Saved {len(names)} vectors  shape={features.shape}  -> {output_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        print("Usage: python extract_clip_h14.py <image_folder> <output_path>")
